...
image = cv2.imread("82.jpg")
pitch2D = cv2.imread("model/sportsfield_release/data/world_cup_template.png")
side = 'right'

optim_homography = torch.tensor([[[ 0.2206, -0.0072,  0.3939],
                                  [ 0.1510,  1.0477, -0.0144],
                                  [-0.0171,  0.7499,  1.0000]]])

defender = [[913, 533], [743, 496], [1311, 605]]
attacker = [[1393, 726], [827, 530]]
...
if side == 'right':
    ...
    pitch2D = cv2.line(pitch2D, (int(p1_2D[0]), int(p1_2D[1])), (int(p2_2D[0]), int(p2_2D[1])), (0, 0, 255), 3)
    ...
    image = cv2.line(image, (int(p1_3D[0]), int(p1_3D[1])), (int(p2_3D[0]), int(p2_3D[1])), (0, 0, 255), 3)
